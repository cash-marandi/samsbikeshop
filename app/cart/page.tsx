      ...
      {itemCount === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            {items.map(item => (
              <div key={item.product._id} className="flex items-center bg-white rounded-lg p-4 border border-gray-300">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-orange-500 font-semibold mt-1">R{(item.product.price * (1 - (item.product.discount || 0) / 100)).toFixed(2)}</p>
                </div>
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => updateItemQuantity(item.product._id!, item.quantity - 1)}
                    disabled={item.quantity <= 1}
                    className="px-3 py-1 bg-gray-200 hover:bg-gray-300 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    -
                  </button>
          ...
          <div className="lg:col-span-1 bg-white rounded-lg p-6 border border-gray-300 h-fit">
