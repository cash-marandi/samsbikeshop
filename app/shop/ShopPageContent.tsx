...
export default function ShopPageContent({ products }: { products: Product[] }) {
  const [filter, setFilter] = useState<ProductType | 'ALL'>('ALL');
  const [search, setSearch] = useState('');
  const { addItem, showToast } = useCart();

  const brands = useMemo(() => {
    return [...new Set(products.map((p: Product) => p.brand))].sort() as string[];
  }, [products]);
  ...
  const filtered = useMemo(() => products
    ...
    [products, filter, selectedBrand, search, priceRange, sortBy]
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="mb-12">
          <p className="text-ink-600 leading-relaxed max-w-3xl">
            Browse our curated collection of premium bicycles, components, and cycling accessories at Sam&apos;s Bike Shop in Soweto, Gauteng. Whether you&apos;re a competitive road cyclist, weekend mountain biker, or looking for the perfect commuter bike, our expert team helps you find the right fit. Every bike in our inventory is inspected by our UCI-certified mechanics and comes with a service guarantee. We stock top brands and offer flexible payment options for cyclists across Johannesburg and South Africa.
          </p>
        </section>
