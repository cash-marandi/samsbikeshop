...
import { Product, Review } from '@/app/types'; // Import Review type
...
export default function ProductDetailPage() {
      ...
      const fetchProductAndReviews = async () => {
        ...
        try {
          ...
          if (!productResponse.ok) {
            ...
            throw new Error(`Failed to fetch product: ${productResponse.status} ${productResponse.statusText} - ${errorText}`);
          }
          ...
          if (session?.user?.id) {
            setHasUserReviewed(reviewsData.some(review => review.userId === session.user?.id));
          }
  ...
  }, [id, session?.user?.id]); // Re-run if session user changes to update review status
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReviews(prev => [result, ...prev]); // Add new review to the top
      setNewReviewRating(0);
      setNewReviewComment('');
      setHasUserReviewed(true); // User has now reviewed

      // Optionally, refetch product to update averageRating/reviewCount
      const productResponse = await fetch(`/api/products/${id}`);
      if (productResponse.ok) {
        const productData: Product = await productResponse.json();
        setProduct(productData);
      } else {
        // Log if product refetch fails, but don't block review submission
        console.error('Failed to refetch product after review submission.');
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-12">
      ...
      <div className="mt-16 border-t border-zinc-800 pt-8">
        ...
        {reviewLoading && <div className="text-xl">Loading reviews...</div>}
        {reviewError && <div className="text-red-500 text-xl">Error: {reviewError}</div>}
